	...
	for (i = 0; i < l; i++) {
		...
		if ("" + node === node) {
			...
			if (tagName === "*") {
				...
				@@if (context.isNode) {// NODE.JS-SPECIFIC CODE:
				var prm = params[1];
...
function getTargetProps(source, tagCtx) {
	...
	if (!propsArr) { // map.propsArr is the full array of {key:..., prop:...} objects
		...
		if (typeof source === OBJECT || $isFunction(source)) {
			for (key in source) {
				prop = source[key];
				if (key !== $@@if (context.isNode) {.}expando && source.hasOwnProperty(key) && (!tagCtx.props.noFunctions || !$.isFunction(prop))) {
...
@@if (!context.isNode) {if (@@if (!context.isJsViews) {!(jsr || $ && $.render)}@@if (context.isJsViews) {!$.link}) }{
