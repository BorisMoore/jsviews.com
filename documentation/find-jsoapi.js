...
content.find.jsoapi = content.useStorage && JSON.parse(localStorage.getItem("JsViewsDocTopics/find/jsoapi")) ||
{
  "jsoapi": {
    "sections": [
      {
  ...
  "observeAll": {
    "sections": [
      ...
      },
      {
        ...
        "text": "$.observable(objectOrArray).observeAll(myHandler)\nRegister an event handler to observe all changes in an object and on any nested object or array in the 'object graph'  under it\nObserve all changes under an object or array\n\n$.observable(person).observeAll(myHandler);\n\nObserve all changes under an object or array, subject to a filter test\n\n$.observable(person).observeAll(myHandler, myFilter);\n\n"
      },
      {
        "_type": "para",
        "title": "",
        "text": "The myHandler function registered using the .observeAll() is identical to the handler used with $.observe() (though it provides an additional ev.data.observeAll helper object):\n"
      },
      {
        "_type": "api",
        "title": "function myHandler(ev, eventArgs) { ... }",
        "text": "function myHandler(ev, eventArgs) { ... }\nAn event handler for observable data changes - registered using $.observable(object).observeAll(myHandler)\nHandler for observable property or array change events\n\n$.observable(person).observeAll(myHandler);\n\nfunction myHandler(ev, eventArgs) {...}\n\n"
      },
      {
        "_type": "para",
        "title": "Sample:",
      ...
      },
      {
        ...
        "text": "Providing a filter method to observeAll\n(Note: This feature is currently deprecated. An improved version may be provided in a future update.)Sometimes a scenario requires that $.observable(rootObject).observeAll(handler) calls the handler for a subset of the observable changes under the rootObject, rather than for every change.\nThis is achieved by passing a filter function as the second parameter of observeAll():\n$.observable(rootObject).observeAll(handler, function(path, object, parentObs) {\n  return ... // Return true for events that should trigger the handler, false for the others...\n});\n\n"
      },
      {
        "_type": "api",
        "title": "function myFilter(path,  object, parentObs) { ... }",
        "text": "function myFilter(path,  object, parentObs) { ... }\nA filter method for observable data changes - registered using $.observable(object).observeAll(myHandler, myFilter)\nFilter method for observeAll() property or array change events\n\n$.observable(person).observeAll(myHandler, myFilter);\n\nfunction myFilter (path, object, parentObs) { ... }\n\n"
      },
      {
        "_type": "para",
        "title": "observeAll() with View Model object hierarchies",
        "text": "observeAll() with View Model object hierarchies\nThe observeAll() API works equally well with View Model objects or plain objects. See JsViews: Data / View Model. At the end of that topic we show modified versions of our observeAll() sample above, using ‘hand-coded’ View Model objects, or using compiled View Model objects.\n"
      },
      {
        "_type": "para",
  ...
  "unobserveAll": {
    "sections": [
      ...
      },
      {
        ...
        "text": "unobserveAll() with View Model object hierarchies\nThe unobserveAll() API works equally well with View Model objects or plain objects. See JsViews: Data / View Model. At the end of that topic we show modified versions of our unobserveAll() sample above, using ‘hand-coded’ View Model objects, or using compiled View Model objects.\n"
      },
      {
        "_type": "links",
        "title": "See also:",
  ...
  "computed": {
    "sections": [
      ...
      },
      {
        ...
        "text": "Dependencies on other computed variables\nOr course a computed observable may depend on both regular (non-computed) properties and other computed observables. (Paths referencing computed observables will have parens, as in lastName().)\nFor example:\nmyObservableFn.depends = [person1, \"lastName()\", \"firstName\", address()^*\", person2, \"address()^street()\"];\n\n"
      },
      {
        "_type": "para",
      ...
      },
      {
        "_type": "para",
        "title": "",
        "text": "See also:\n\nthe Tag depends option\nthe Using the {^{for}} tag as a ‘purchases’ grid control sample\n\n"
      },
      {
        "_type": "para",
    ...
    "sections": [
      {
        ...
        "text": "Asynchronous $.observable(...) property and array change events\nThe $.observable(object)... or $.observable(array)... calls accept an additional delay parameter, which can be used to delay the triggering of the associated observable change event, either to make it asynchronous, or to prevent automatic triggering, and to allow complete specific control over whether or when the subsequent triggering of the event occurs (such as for batched events).\n$.observable(myObject, delay).setProperty(...);\n\n$.observable(myArray, delay).insert(...);\n\n"
      },
      {
        "_type": "para",
        "title": "Async observable change events",
        "text": "Async observable change events\nBy setting delay to true, the observable change event(s) will be triggered asynchronously:\n$.observable(myObject, true).setProperty(...);\n$.observable(myArray, true).insert(...);\n// ... other synchronous code ...\n\nNote that in the example above, the property changes to myObject and the insertion of an item in myArray will still occur synchronously. But the corresponding observable change events will be raised asynchronously (so will be triggered after the ... other synchronous code ..., above).\nIf multiple changes are made to the same property, then the raised events will include only one event for that property, corresponding to the setting of the final value. This can provide performance improvement in some scenarios…\nA common scenario for asynchronous observable change events is when mouse move events are used to trigger observable changes, such as for {{slider}} tag controls.\nSee also the updateValue() and updateValues() methods, which provide the same delay options for the raised events, discussed in the topic Async or batched change events, with tag.updateValue() or tag.updateValues().\n"
      },
      {
        "_type": "para",
        "title": "Batched observable change events",
        "text": "Batched observable change events\nBy passing in an array as delay parameter, the observable change events will not be triggered at all, but will be added (as data structures) to the array. The array will now have a trigger method available. Subsequent calling of the trigger method will then raise the corresponding events.\nThis allows complete control over whether or when the subsequent triggering of the event occurs.\nBy passing in different arrays (such as myBatch1 and myBatch2) to different calls, the triggering of the events can be ordered and timed as desired in different batches…\nExample:\nvar myBatch1 = [];\nvar myBatch2 = [];\n\n// Change person.name property, without raising property change event\n$.observable(person, myBatch1).setProperty(\"name\", \"newName\");\n\n// Change team.title and team.description properties, without raising property change events\n$.observable(team, myBatch2).setProperty({title: ..., description: ...});\n\n// Add a new member(s) to team.members, without raising array change events\n$.observable(team.members, myBatch1).insert(...);\n\n// Now trigger all the above property and array change events, in a specific order.\nmyBatch1.trigger();\nmyBatch2.trigger();\n\nNote that with batched observable change events (just as for asynchronous events) the property changes and array changes still occur synchronously, but the corresponding observable change events are delayed until the trigger() method is called on the delay array.\nAlso, just as for asynchronous events, if multiple changes are made to the same property then the raised events will include only one event for that property, corresponding to the setting of the final value. Again, this can provide performance improvement in some scenarios…\n"
      },
      {
        "_type": "para",
        "title": "Sample: Delayed observable change events",
        "text": "Sample: Delayed observable change events\nThe following sample illustrates the different behaviors of delayed observable change events, with delay parameter set to false (synchronous), true (asynchronous) or an array batch = [] (batched events with programmatic trigger):\n"
      },
      {
        "_type": "sample",
        "title": "",
      ...
      },
      {
        "_type": "para",
        "title": "",
        "text": "Note: Delayed events (async or batched) can be also made to target specific event namespaces, – simply by including the appropriate namespace parameter in the property change or array change call:\n$.observable(namespace, object, delay).setProperty(...);\n$.observable(namespace, array, delay).insert(...);\n\nSee also:\n\nAsync or batched change events, with tag.updateValue() or tag.updateValues()\n\n"
      },
      {
        "_type": "para",
