...
content.find.jsrapi = content.useStorage && JSON.parse(localStorage.getItem("JsViewsDocTopics/find/jsrapi")) ||
{
  "jsrapi": {
    "sections": [
      {
  ...
  "contextualparams": {
    "sections": [
      ...
      },
      {
        ...
        "text": "Accessing root data: the built-in '~root' contextual parameter\nThe built-in contextual parameter ~root provides direct access to the root data which was passed to the render() method (or link() method if you are using JsViews). It can be accessed from anywhere within a template, at any level of nested tags.\nNote: If an array is passed to render() or link() then ~root will be the array (so you can render {{:~root.length}} for example).\n"
      },
      {
        "_type": "links",
        "title": "See also:",
