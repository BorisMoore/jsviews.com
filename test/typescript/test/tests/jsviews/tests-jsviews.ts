...
function log(code, message?, success?) {
	message = message || '';
...
let assert = {
	equal: function (a, b, code, message?) {
		log(code, message || '', a === b);
	},
	ok: function (a, code, message?) {
		log(code, message || '', a);
	},
	testGroup: function (message) {
		log(undefined, message, undefined);
	}
};
...
/*<<<<<<<<<<<<<*/ assert.testGroup("template.useViews"); /*>>>>>>>>>>>>>*/
	...
	assert.ok($.templates("{{for/}}").useViews === false, '$.templates("{{for/}}").useViews' , "useViews defaults to false");
...
/*<<<<<<<<<<<<<*/ assert.testGroup("$.views.helpers()"); /*>>>>>>>>>>>>>*/
	...
	let helper = $.views.helpers("add", function(val1, val2) { return val1 + "|" + val2 + ", "; });

	test = helper("a", "b");

	tmpl = $.templates("{{:~add(first, last)}} {{:~privateadd(first, last)}}");

	helper = $.views.helpers({privateadd: function(val1, val2) { return val1 + "!!" + val2 + ", "; }}, tmpl)
...
/*<<<<<<<<<<<<<*/ assert.testGroup("$.views.viewModels()"); /*>>>>>>>>>>>>>*/
	...
	let Book = $.views.viewModels({
		getters: ["title", "price"],
		extend: {nameAndPrice:  function(reverse?) {
	...
	let MyVMs = {} as JsViews.Hash<JsViews.ViewModel>;
...
// The following code is from sample: https://www.jsviews.com/#viewmodelsapi@mergesampleadv plus use of parentRef
	...
	let myVmCollection = {} as JsViews.Hash<JsViews.ViewModel>;;
...
/*<<<<<<<<<<<<<*/ assert.testGroup("view"); /*>>>>>>>>>>>>>*/
	...
	$.views.tags("mytag", {
		...
		do: function(tagCtx, ev, eventArgs) {
			data3.a = "A2";
...
/*<<<<<<<<<<<<<*/ assert.testGroup("Observable"); /*>>>>>>>>>>>>>*/
	...
	let data4 = {person: {name: "Jo", address: {street: "Main St"}, phones: [1, 2, 3], info: null}};

	data4.person.info = function() {
